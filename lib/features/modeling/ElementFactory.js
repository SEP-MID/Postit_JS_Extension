...
//Breite und Höhe der Elemente wird hier angepasst 
//Standardmässig haben alle Handler im Postit Renderer die Höhe und Breite: element.width und element.height, diese wird hier übergeben 
//Gedanke: Den Wert des Pfades dynamisch zur Laufzeit übergeben 
// --> Anpassung im Renderer benötigt


ElementFactory.prototype._getDefaultSize = function(semantic) {
   
  if (is(semantic, 'postit:HappyEmotionPostit')) {
    return { width: 32, height: 32 };
  }

  if (is(semantic, 'postit:NeutralEmotionPostit')) {
    return { width: 32, height: 32 };
  }

  if (is(semantic, 'postit:SadEmotionPostit')) {
    return { width: 32, height: 32 };
  }

  if (is(semantic, 'postit:TruthPostit')) {
    return { width: 25, height: 50 };
  }

  if (is(semantic, 'postit:ContentPostit')) {
    return { width: 200, height: 1050 };
  }


  if (is(semantic, 'postit:CirclePostit')) {
    return { width: 40, height: 40 };
  }

  if (is(semantic, 'postit:SquarePostit')) {
